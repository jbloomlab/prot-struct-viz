...
- **One input: the YAML spec.** `prot-struct-viz spec.yaml` is the whole CLI bar one
  flag, `--out`: every other path in a spec names an input sitting beside it, while the
  output path points out of that directory, so it belongs to whoever owns the output tree.
  `--out` resolves relative to the working directory and is mutually exclusive with the
  spec's `out` — exactly one must be given, enforced in `load_spec`, which both the CLI
  and the Python API pass through. `spec.load_spec` parses the file into a `Spec` of
  `View`s, each a name, a CSV, and a `ViewConfig`, and `render(spec)` takes it from there.
  A spec key, a `ViewConfig` field name and a `docs/spec.md` row all use the same name;
  `spec.OPTION_KEYS` is that list, built from the loader's own key tuples so it cannot
  fall behind them. **What an option means is written once, in `docs/spec.md`** — the
  page a reader actually gets — and nowhere in the source. Adding an option means a
  `ViewConfig` field, an entry in the relevant `spec.py` key tuple, and a row in
  `docs/spec.md`; `tests/test_docs.py` checks every key reaches that page.
